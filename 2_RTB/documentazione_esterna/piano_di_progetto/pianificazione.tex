...
\begin{itemize}
	\item \textbf{Maggiore flessibilità e adattabilità}: Scrum permette di rispondere rapidamente ai cambiamenti nei requisiti del progetto o nelle condizioni di mercato. Le iterazioni brevi (Sprint) consentono di rivedere e adattare frequentemente il piano di lavoro;
	\item \textbf{incremento della produttività}: suddividendo il lavoro in Sprint brevi e gestibili, il team può concentrarsi su obiettivi specifici, migliorando la produttività e riducendo il tempo necessario per consegnare i risultati;
	      \begin{itemize}
	...
	\item \textbf{maggiore trasparenza}: le pratiche di Scrum, come le riunioni settimanali, le revisioni di Sprint e le retrospettive, garantiscono una comunicazione costante e trasparente all'interno del team e con gli stakeholder;
	\item \textbf{coinvolgimento e motivazione del team}: la responsabilità condivisa e l’autonomia dei team Scrum promuovono un ambiente di lavoro più motivante e coinvolgente, dove i membri del team sono incoraggiati a contribuire attivamente e a prendere decisioni;
	...
	\item \textbf{maggiore soddisfazione del cliente}: coinvolgendo i clienti e gli stakeholder nel processo di sviluppo e fornendo incrementi di prodotto funzionanti a intervalli regolari, Scrum garantisce che il prodotto finale risponda meglio alle esigenze e alle aspettative del cliente;
	...
	\item \textbf{migliore previsione e pianificazione}: grazie a Sprint di durata fissa e al monitoraggio costante del progresso, è più facile fare previsioni accurate sui tempi e sui costi del progetto;
	\item \textbf{feedback continuo}: il ciclo di feedback rapido e continuo da parte del cliente e degli stakeholder consente di apportare miglioramenti costanti e di allineare il prodotto alle esigenze del mercato in evoluzione.
\end{itemize}
...
\begin{table}[!h]
	...
	\begin{tabular}{ | p{6cm} | p{2.5cm} | p{7.5cm} | }
		...
		\hline
		Inesperienza nell'uso delle tecnologie adottate (Rischio \textbf{RT-3}). & SI                            & Autoformazione e discussione interna e con la proponente.           \\
		\hline
		Ritardi rispetto alle tempistiche previste (Rischio \textbf{RO-3}).      & NO                            & Anticipati alcuni compiti previsti per il prossimo sprint.          \\
		\hline
	\end{tabular}
...
\begin{table}[!h]
	...
	\begin{tabular}{ | p{6cm} | p{2.5cm} | p{7.5cm} | }
		...
		\textbf{Tipologia}                                                       & \textbf{Rischio preventivato} & \textbf{Mitigazione}                                                               \\
		\hline
		Inesperienza del team (Rischio \textbf{RO-1}).                           & SI                            & Ridistribuzione compiti assegnati durante lo sprint.                               \\
		\hline
...
\begin{figure}[!h]
	...
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Impegno preventivo per ruolo dei membri del team durante il quarto sprint}
...
\begin{figure}[!h]
	...
	\end{tikzpicture}
	\caption{Ripartizione in percentuale dei ruoli nel quarto sprint}
	
\end{figure}

\newpage
\subsubsubsection{Consuntivo}
In questo sprint siamo riusciti a portare a termine tutte le attività previste.
...
\begin{table}[!h]
	...
	\begin{tabular}{ | l | c | c | c | c | c | }
		...
		\textbf{Ruolo}             & \textbf{Ore} & \textbf{Costo} & \textbf{Ore rimanenti preventivo} & \textbf{Ore rimanenti consuntivo} \\
		\hline
		Responsabile               & 3            & 90,00 €        & 36                                & 37                                \\
		Amministratore             & 4            & 80,00 €        & 35                                & 35                                \\
...
\begin{itemize}
    ...
    %TODO completare fine sprint
    % \item Fine attuale: - .
    % \item Giorni di ritardo: - .
\end{itemize}

\subsubsubsection{Pianificazione}
Visto il livello di avanzamento del progetto, il gruppo intende candidarsi alla prima revisione di avanzamento (RTB). Per questo motivo, il team concentrerà maggiori risorse nella revisione e approvazione finale dei documenti necessari per la consegna. Inoltre, sarà ultimato il PoC, fondamentale per affrontare la revisione. In particolare, ci si concentrerà su:
\begin{itemize}
	\item revisione e approvazione \textit{Piano di Progetto};
	\item revisione e approvazione \textit{Piano di Qualifica};
	\item revisione e approvazione \textit{Analisi dei Requisiti};
	\item revisione e approvazione \textit{Norme di Progetto};
	\item miglioramento della visualizzazione delle dashboard su \textit{Grafana}.
\end{itemize}
...
\begin{table}[!h]
    ...
    \begin{tabular}{ | l | c | c | c | c | c | c | c | }
        ...
        \hline
    \end{tabular}
    \caption{Preventivo orario per ruolo dei membri del team durante il quinto sprint} 
\end{table}
...
\begin{figure}[!h]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Impegno preventivo per ruolo dei membri del team durante il quinto sprint} 
\end{figure}
...
\begin{figure}[!h]
    ...
    \begin{tikzpicture}
        ...
        \end{tikzpicture}
    \caption{Ripartizione in percentuale dei ruoli nel quinto sprint} 
\end{figure}
...
%         \hline
%     \end{tabular}
%     \caption{Consuntivo orario per ruolo dei membri del team durante il quinto sprint} % TODO mettere numero sprint corretto
% \end{table}
...
%         \hline
%     \end{tabular}
%     \caption{Consuntivo economico consuntivo durante il quinto sprint} 
% \end{table}
...
%         \hline
%     \end{tabular}
%     \caption{Rischi effettivamente occorsi e loro mitigazione durante il quinto sprint} % TODO mettere numero sprint corretto
%     
% \end{table}
...
% \newpage
% \subsubsection{Sesto sprint:} % TODO mettere numero di sprint corretto
% \begin{itemize}
...
%         \hline
%     \end{tabular}
%     \caption{Preventivo orario per ruolo dei membri del team durante il quinto sprint} % TODO mettere numero di sprint corretto
%     
% \end{table}
%
% \newpage
...
%         \end{axis}
%     \end{tikzpicture}
%     \caption{Impegno preventivo per ruolo dei membri del team durante il quinto sprint} % TODO mettere numero sprint corretto
%     
% \end{figure}
%
% %---------1_GRAFICO A TORTA-----------%
...
%         }
%         \end{tikzpicture}
%     \caption{Ripartizione in percentuale dei ruoli nel quinto sprint} % TODO mettere numero sprint corretto
% \end{figure}
%
% \newpage
% \subsubsubsection{Consuntivo}
...
%         \hline
%     \end{tabular}
%     \caption{Consuntivo economico consuntivo durante il quinto sprint} % TODO mettere numero sprint corretto
